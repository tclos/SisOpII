...
Client::Client(int port) //Inicializa o cliente e define o ID da requisição como 1
    ...
    std::cout << getCurrentFormattedTime() 
              << " [CLIENTE] Cliente inicializado na porta " << port 
              << std::endl;
}
...
std::string Client::discoverServer() { 
    std::cout << getCurrentFormattedTime() 
              << " [CLIENTE] Procurando servidor na rede..." << std::endl;
    ...
    std::cout << getCurrentFormattedTime() 
              << " [CLIENTE] Servidor encontrado: " << this->server_address 
              << std::endl;
...
std::pair<bool, AckData> Client::executeRequestWithRetries(const std::string& dest_ip, int value) {
    ...
    std::cout << getCurrentFormattedTime() 
              << " [CLIENTE] Executando requisição #" << sequence_number 
              << " para " << dest_ip << " valor: " << value << std::endl;
    ...
    while (!ack_received && retries < MAX_RETRIES) {
        ...
        if (!sendRequestPacket(sequence_number, dest_ip, value, client_socket, current_server_addr)) {
            std::cerr << getCurrentFormattedTime() 
                      << " [CLIENTE] Aviso: Falha no envio do pacote." << std::endl;
        }
        ...
        if (receiveResponse(response_packet, sequence_number, client_socket)) {
            ...
            retries++;
            std::cout << getCurrentFormattedTime() 
                      << " [CLIENTE] Timeout! Reenviando requisição #" << sequence_number
                      << " (" << retries << "/" << MAX_RETRIES << ")" << std::endl;
        }
    }

    std::cerr << getCurrentFormattedTime() 
              << " [CLIENTE] ERRO: Máximo de retentativas (" << MAX_RETRIES 
              << ") alcançado para requisição #" << sequence_number << std::endl;
...
void Client::incrementSequenceNumber() {
    ...
    std::cout << getCurrentFormattedTime() 
              << " [CLIENTE] Próximo ID de requisição: " << this->sequence_number 
              << std::endl;
}

Client::~Client() {
    std::cout << getCurrentFormattedTime() 
              << " [CLIENTE] Encerrando cliente..." << std::endl;
    ...
    std::cout << getCurrentFormattedTime() 
              << " [CLIENTE] Cliente encerrado." << std::endl;
}
...
void Client::listenForAnnouncements() {
    ...
    if (sockfd < 0) {
        std::cerr << getCurrentFormattedTime() 
                  << " [CLIENTE] ERRO: Não foi possível criar socket para anúncios" 
                  << std::endl;
        return;
    }
    ...
    if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
        perror("[CLIENTE] Setsockopt falhou no listener do cliente");
        close(sockfd);
        return;
    }
    ...
    if (bind(sockfd, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
        perror("[CLIENTE] Bind falhou no listener do cliente");
        close(sockfd);
        return;
    }

    std::cout << getCurrentFormattedTime() 
              << " [CLIENTE] Escutando anúncios de novos servidores primários..." 
              << std::endl;
    ...
    while (running) {
        ...
        if (n > 0) {
            if (ntohs(packet.type) == NEW_PRIMARY_ANNOUNCEMENT) {
                ...
                uint32_t new_server_id = ntohl(packet.data.req.value);
                
                std::lock_guard<std::mutex> lock(server_mutex);
                
                if (new_ip != this->server_address) {
                    std::cout << getCurrentFormattedTime() 
                              << " [CLIENTE] NOVO SERVIDOR PRIMÁRIO anunciado: " 
                              << new_ip << " (ID: " << new_server_id << ")" << std::endl;
                    ...
                    std::cout << getCurrentFormattedTime() 
                              << " [CLIENTE] Servidor primário atualizado para: " 
                              << this->server_address << std::endl;
                }
            }
        }
    }
    close(sockfd);
    std::cout << getCurrentFormattedTime() 
              << " [CLIENTE] Listener de anúncios encerrado." << std::endl;
}